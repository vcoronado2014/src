...
export class ServicioGeo {
  ...
  getPreguntas(run) {
    let url = environment.API_ENDPOINT + 'DatosInscripcion?run=' + run;
    let data = this.httpClient.get(url, {});
    return data;
  }
  getPreguntasNative(run) {
    let url = environment.API_ENDPOINT + 'DatosInscripcion?run=' + run;
    let data = this.http.get(url, {}, {});
    return data;
  }
  getRegistroCorreoRun(correo, run) {
    let url = environment.API_ENDPOINT + 'ValidaCorreo?run=' + run + '&correo=' + correo;
    let data = this.httpClient.get(url, {});
    return data;
  }
  getRegistroCorreoRunNative(correo, run) {
    let url = environment.API_ENDPOINT + 'ValidaCorreo?run=' + run+ '&correo=' + correo;
    let data = this.http.get(url, {}, {});
    return data;
  }
  //para nuevo registro de usuario
  verificaEnrolamientoRegistro(run, email) {
    let url = environment.API_ENDPOINT + 'PreRegistroApp?Run=' + run + '&Email=' + email;
    let data = this.httpClient.get(url, {});
    return data;
  }
  verificaEnrolamientoRegistroNative(run, email) {
    let url = environment.API_ENDPOINT + 'PreRegistroApp?Run=' + run  + '&Email=' + email;
    let data = this.http.get(url, {}, {});
    return data;
  }
